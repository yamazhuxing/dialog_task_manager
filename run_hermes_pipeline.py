...
# -*- coding: utf-8 -*-
"""
Hermes 一键流水线：格式转换 → 质检 → 难度评级

用法：
  uv run python run_hermes_pipeline.py --input_dir sample/hermes
  uv run python run_hermes_pipeline.py --input_dir sample/hermes --skip_rate
  uv run python run_hermes_pipeline.py --input_dir sample/hermes --api_key sk-xxx
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Hermes 一键流水线：转换 → 质检 → 难度评级",
...
示例:
  ...
  uv run python run_hermes_pipeline.py --input_dir sample/hermes --skip_rate
    ...
    parser.add_argument("--input_dir", required=True, help="Hermes 原始 .json 所在目录")
    ...
    run_step("1/3 格式转换 (convert_hermes.py)", "convert_hermes.py", convert_args)
